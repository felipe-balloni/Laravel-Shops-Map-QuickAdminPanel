        ...
        <form method="POST" action="{{ route("admin.shops.update", [$shop->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('categories') ? 'is-invalid' : '' }}"
                    ...
                    <option value="{{ $id }}"
                        {{ (in_array($id, old('categories', [])) || $shop->categories->contains($id)) ? 'selected' : '' }}>
                        {{ $categories }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control map-input {{ $errors->has('address') ? 'is-invalid' : '' }}" type="text"
                    name="address" id="address" value="{{ old('address', $shop->address) }}">
                <input type="hidden" name="latitude" id="address-latitude"
                    value="{{ old('latitude', $shop->latitude) ?? '0' }}" />
                <input type="hidden" name="longitude" id="address-longitude"
                    value="{{ old('longitude', $shop->longitude) ?? '0' }}" />
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="active" id="active" value="1"
                        {{ $shop->active || old('active', 0) === 1 ? 'checked' : '' }}>
            ...
            <table class="table table-striped w-50" >
                ...
                <tbody>
                    @foreach($days as $day)
                    <tr>
                        <th scope="row">{{ ucfirst($day->name) }}</th>
                        <td>
                            <div class="form-inline">
                                ...
                                <select class="custom-select my-1 mr-sm-2" name="from_hours[{{ $day->id }}]">
                                    ...
                                    <option value="{{ $hours < 10 ? "0$hours" : $hours }}"
                                ...
                                <select class="custom-select my-1 mr-sm-2" name="from_minutes[{{ $day->id }}]">
                                    <option value="">--</option>
                                    <option value="00"
                                        {{ old('from_minutes.'.$day->id, $shop->days->find($day->id) ? $shop->days->find($day->id)->pivot['from_minutes'] : null) == '00' ? 'selected' : '' }}>
                                        00</option>
                                    <option value="30"
                                        {{ old('from_minutes.'.$day->id, $shop->days->find($day->id) ? $shop->days->find($day->id)->pivot['from_minutes'] : null) == '30' ? 'selected' : '' }}>
                                        30</option>
                                </select>
                            </div>
                        </td>
                        <td>
                            <div class="form-inline">
                                ...
                                <select class="custom-select my-1 mr-sm-2" name="to_minutes[{{ $day->id }}]">
                                    <option value="">--</option>
                                    <option value="00"
                                        {{ old('to_minutes.'.$day->id, $shop->days->find($day->id) ? $shop->days->find($day->id)->pivot['to_minutes'] : null) == '00' ? 'selected' : '' }}>
                                        00</option>
                                    <option value="30"
                                        {{ old('to_minutes.'.$day->id, $shop->days->find($day->id) ? $shop->days->find($day->id)->pivot['to_minutes'] : null) == '30' ? 'selected' : '' }}>
                                        30</option>
                                </select>
                            </div>
                        </td>
...
@section('scripts')
<script
    src="https://maps.googleapis.com/maps/api/js?key={{ env('GOOGLE_MAPS_API_KEY') }}&libraries=places&callback=initialize&language=pt-BR&region=BR"
    async defer></script>
